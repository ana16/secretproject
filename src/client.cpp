...
int getGameCode(){
    ...
    cout << "printing buffer index::\n";
    for(int i = 0; i < bufferVector.size(); i++){
        
        
        cout << "index: " << i << " buffer: " << bufferVector[i] << endl;
        
    }
    
    
    
    std::string strbuf = bufferVector[0];
     ...
     inputVector[4] == "WILL" && inputVector[5] == "PLAY" && (inputVector[7] == "MATCH" || inputVector[7] == "MATCHES")){
       ...
       myBoard2 = new Board();
        ...
        if(inputVector[5] == "1"){
            ...
            cout << "trying to place tileNum in 1: " << newTile->tileNum << endl;
...
//              cout << "tileNum: " << newTile->tileNum << endl;
        ...
        //            tempString = patch::to_string(40);
                ...
                if(otherPlayer == "TEAMP"){

                    myBoard1->exportBoardState();
                }

        }
        else if(inputVector[5] == "2"){
            cout << "in game 2\n";
            int tileNum = conv2int(inputVector[12]);

            Tile* newTile =  myDeck->getTile(tileNum);
            
            cout << "trying to place tileNum in 2: " << newTile->tileNum << endl;
...
//              cout << "tileNum: " << newTile->tileNum << endl;
    ...
    //            tempString = patch::to_string(40);
        ...
        cout << "our move: " << outputString << endl;
        ...
        cout << "otherplayer is: " << otherPlayer << endl;
    cout << "inputVector check other player is: " << inputVector[5] << endl;
        if(inputVector[5] == otherPlayer){
            ...
            if(inputVector[1] == "1"){
                ...
                myBoard1->addTile(newTile,tempX + 40,40 - tempY);
                bzero(buffer,1000);


            }
            else if(inputVector[1] == "2"){

                myBoard2->addTile(newTile,tempX + 40,40 - tempY);
                bzero(buffer,1000);
...
int main(int argc, char *argv[])
{
    ...
    // 1.  n = read(sockfd, buffer, 999);
    // 2.  n = write(sockfd, buffer, strlen(buffer));
    // make sure to clear buffer with:  bzero(buffer,1000);      
    ...
    //     cout << "got in while loop\n";
    //     //new challenge
    //     int matchNum = read(sockfd, buffer, 999);
    ...
    //         bzero(buffer,1000);
        ...
        std::vector<std::string> tempVector;
            ...
            while(true){
                ...
                cout << "tempstring outside: " << tempString << endl;
                cout << "size of bufferVector: " << bufferVector.size() << endl;
                ...
                while(tempVector.size()>0){
                    ...
                    cout << "loop in while for a while\n";
                    cout << "tempvect size: " << tempVector[0].size();
                    if(tempVector[0].size() > 2){
                        // cout << "tempstring outside: " << tempString << endl;

                            bufferVector.push_back(tempVector[0]);
...
string conv2str(int x){
 ...
 // std::string newstrbuf(buffer);
                // std::cout << "strbuf on outside: " << newstrbuf << std::endl;
                
                // split(newstrbuf,'\n',trueInput);
                
                // cout << "trueInputsize: " << trueInput.size() << endl;
                
                // if(trueInput.size() > 1){
                    
                //     strcpy(buffer, trueInput[0].c_str());
                    
                //     getGameCode();
                    
                //     strcpy(buffer, trueInput[1].c_str());
